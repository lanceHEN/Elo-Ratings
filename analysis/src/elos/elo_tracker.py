...
from utils import basic_win_prob_for_et, elo_update
...
class EloTracker:
    ...
    def add_history(
        self, game_df: pd.DataFrame, add_elos_to_df=False, add_win_probs_to_df=False
    ) -> None:
        ...
        for _, game in game_df.iterrows():
            ...
            initial_home_elo, home_wins, home_losses = self._get_initial_team_stats(
                home_team, season
            )
            initial_away_elo, away_wins, away_losses = self._get_initial_team_stats(
                away_team, season
            )
            ...
            mov = (
                game[self.margin_of_victory_column]
                if self.margin_of_victory_column
                else None
            )

            updated_home_elo, updated_away_elo = elo_update(
                initial_home_elo, initial_away_elo, home_won, home_win_prob, self.K, mov
            )
